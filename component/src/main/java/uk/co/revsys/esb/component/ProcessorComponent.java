...
public abstract class ProcessorComponent extends DefaultComponent {
    ...
    protected Endpoint createEndpoint(String uri, String remaining, Map<String, Object> parameters) throws Exception {
        ...
        if (processor instanceof ParameterAwareProcessor) {
            setProperties(processor, parameters);
        }
        if(processor instanceof ParameterMapAwareProcessor){
            ((ParameterMapAwareProcessor)processor).setParameters(parameters);
        }
        return endpoint;
    }
